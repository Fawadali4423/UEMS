...
class _CreateEventScreenState extends State<CreateEventScreen> {
  ...
  Future<void> _handleCreate() async {
    ...
    final event = EventModel(
      id: _isEditMode ? _editingEvent!.id : '', // ID will be generated by repository or ignored for create
      title: _titleController.text.trim(),
      description: _descriptionController.text.trim(),
      date: _selectedDate,
      startTime: '${_startTime.hour.toString().padLeft(2, '0')}:${_startTime.minute.toString().padLeft(2, '0')}${_startTime.period == DayPeriod.am ? " AM" : " PM"}',
      endTime: '${_endTime.hour.toString().padLeft(2, '0')}:${_endTime.minute.toString().padLeft(2, '0')}${_endTime.period == DayPeriod.am ? " AM" : " PM"}',
      venue: _venueController.text.trim(),
      organizerId: authProvider.currentUser?.uid ?? '',
      organizerName: authProvider.currentUser?.name ?? '',
      status: _isEditMode ? _editingEvent!.status : 'pending',
      eventType: _eventType,
      entryFee: entryFee,
      posterBase64: _imageBase64, // Use Base64 as requested
      posterUrl: null, // Reverted to null
      certificateTemplateBase64: _isEditMode ? _editingEvent?.certificateTemplateBase64 : null,
      templateConfig: _isEditMode ? _editingEvent?.templateConfig : null,
      participantCount: _isEditMode ? _editingEvent!.participantCount : 0,
